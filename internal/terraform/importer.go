...
// ImportScriptExists checks whether import.sh exists in the working directory.
func ImportScriptExists(workDir string) bool {
	info, err := os.Stat(filepath.Join(workDir, "import.sh"))
	return err == nil && !info.IsDir()
}

// RunImportScript runs the import.sh script generated by OpenCode.
// It makes the script executable and runs it via bash.
func RunImportScript(workDir string) (string, error) {
	scriptPath := filepath.Join(workDir, "import.sh")

	// Make the script executable.
	if err := os.Chmod(scriptPath, 0o750); err != nil {
		return "", fmt.Errorf("chmod import.sh: %w", err)
	}
	...
	output := out.String()
	if err != nil {
		return output, fmt.Errorf("import.sh failed: %w\n%s", err, output)
	}
	return output, nil
}
